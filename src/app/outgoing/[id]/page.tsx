...
export default async function ViewOutgoingLetterPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold">Chiquvchi xat ma'lumotlari</h1>
      ...
      <div className="bg-white p-8 rounded-lg shadow space-y-6">
        <div className="grid grid-cols-2 gap-6 border-b pb-6">
          ...
          <div>
            <p className="text-sm text-gray-500">Qabul qiluvchi tashkilot</p>
            <p className="font-semibold">{letter.receiverOrg || "-"}</p>
          </div>
          <div>
          ...
          <div>
            <p className="text-sm text-gray-500">Xat turi</p>
            <p className="font-semibold">{letter.outgoingType || "-"}</p>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="font-semibold">{letter.comment || "-"}</p>
        </div>

        {letter.fileName && (
          <div className="pt-6 border-t print:hidden">
            <p className="text-sm text-gray-500 mb-2">Ilova qilingan fayl:</p>
            <a href={`/api/uploads/${letter.fileName}`} target="_blank" className="inline-flex items-center space-x-2 bg-purple-100 text-purple-700 px-4 py-2 rounded hover:bg-purple-200">
