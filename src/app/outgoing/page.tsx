...
export default async function OutgoingLetters({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden p-6 mb-6">
        <form method="GET" action="/outgoing" className="flex gap-4">
          <input 
            type="text" 
            name="q" 
            defaultValue={q} 
          ...
          {q && (
            <Link href="/outgoing" className="bg-red-500 text-white px-4 py-2 rounded shadow hover:bg-red-600">
              Tozalash
            </Link>
          )}
        </form>
            ...
            {letters.map((letter, idx) => (
              <tr key={letter.id} className="border-b hover:bg-gray-50">
                ...
                <td className="p-3 space-x-2 text-sm">
                  ...
                  <Link href={`/outgoing/${letter.id}`} className="text-blue-600 hover:underline">Ko'rish</Link>
                  <Link href={`/outgoing/${letter.id}/edit`} className="text-yellow-600 hover:underline">Tahrirlash</Link>
