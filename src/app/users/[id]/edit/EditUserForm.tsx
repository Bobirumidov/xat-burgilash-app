  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow max-w-lg space-y-4">
      ...
      <div>
        ...
        <input required defaultValue={user.name} name="name" type="text" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700">Login</label>
        <input required defaultValue={user.email} name="email" type="text" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700">Parol</label>
        <input required defaultValue={user.password} name="password" type="text" className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2" />
      </div>
      <div>
        <label className="block text-sm font-medium text-gray-700">Huquq (Rol)</label>
        <select name="role" defaultValue={user.role} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm border p-2">
      ...
      <div className="border-t pt-4">
        ...
        {user.avatar && user.avatar !== "null" && user.avatar !== "undefined" && (
          <div className="mt-2 flex items-center gap-2">
            <ImageModal src={`/api/uploads/${user.avatar}`} alt="Avatar" className="w-10 h-10 rounded-full object-cover border" />
